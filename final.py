...
# إعدادات الصفحة
st.set_page_config(page_title="مولّد وصفات راقٍ", page_icon="🍽️", layout="centered")

# مفاتيح الترجمة
lang_labels = {
    "fr": {
        "title": "🍽️ Générateur de recettes intelligent",
...
# عرض العنوان بشكل جذاب
st.markdown(f"""
<div style="text-align:center; background-color:#f0f2f6; padding:20px; border-radius:10px; margin-bottom:20px;">
    <h1 style="color:#ff4b4b;">{t['title']}</h1>
    <p style="color:#555;">{'Créez des recettes personnalisées en un clic' if lang == 'fr' else 'أنشئ وصفات مخصصة بنقرة واحدة'}</p>
</div>
""", unsafe_allow_html=True)

# تبويبين لاختيار المكونات أو كتابتها
tab1, tab2 = st.tabs([t["select_ingredients"], t["write_ingredients"]])

# تبويب اختيار المكونات من القائمة
with tab1:
...
MODEL_NAME = "gemini-pro" if "gemini-pro" in available_models else "gemini-1.5-pro"
    ...
    if "ingredients" not in st.session_state or len(st.session_state.ingredients) < 2:
        ...
        with st.spinner(t["loading"]):
            ...
            try:
                ...
                <h3 style="color:#ff4b4b; text-align:center;">{recipe_text.split('\n')[0]}</h3>
